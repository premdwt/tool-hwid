...
class Program
{
    ...
    static void Main()
    {
        ...
        while (true)
        {
            ...
            Console.WriteLine("=================================================");
            Console.ResetColor();
            Console.Write("Pilih menu eksekusi (0-13): ");
            ...
            {
                AnimasiProgressBar("[~] Membersihkan folder Temp...");
                BersihkanFolder(Path.GetTempPath()); BersihkanFolder(@"C:\Windows\Temp");
                CetakSukses("File Temp berhasil dibersihkan!");
            }
            if (pilihan == "2" || pilihan == "10")
            {
                AnimasiProgressBar("[~] Mengosongkan Recycle Bin...");
                SHEmptyRecycleBin(IntPtr.Zero, null, SHERB_NOCONFIRMATION);
                CetakSukses("Recycle Bin berhasil dikosongkan!");
            }
            if (pilihan == "3" || pilihan == "10")
            {
                AnimasiProgressBar("[~] Membersihkan Browser Cache...");
                string localAppData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
                BersihkanFolder(Path.Combine(localAppData, @"Google\Chrome\User Data\Default\Cache"));
                BersihkanFolder(Path.Combine(localAppData, @"Microsoft\Edge\User Data\Default\Cache"));
                BersihkanFolder(Path.Combine(localAppData, @"BraveSoftware\Brave-Browser\User Data\Default\Cache"));
                CetakSukses("Cache Browser berhasil dibersihkan!");
            }
            ...
            if (pilihan == "6" || pilihan == "10")
            {
                AnimasiProgressBar("[~] Membumihanguskan cache COD Warzone...");
                ...
                HapusFolderPaksa(Path.Combine(local, "Activision")); HapusFolderPaksa(Path.Combine(local, "Battle.net")); HapusFolderPaksa(Path.Combine(local, "Blizzard Entertainment"));
                HapusFolderPaksa(Path.Combine(app, "Battle.net"));
                HapusFolderPaksa(Path.Combine(prog, "Activision")); HapusFolderPaksa(Path.Combine(prog, "Battle.net")); HapusFolderPaksa(Path.Combine(prog, "Blizzard Entertainment"));
                CetakSukses("Folder Activision & Battle.net musnah total!");
            }
            if (pilihan == "7")
            {
                AnimasiProgressBar("[~] Melakukan Flush DNS...");
                JalankanPerintahSistem("ipconfig", "/flushdns", true); JalankanPerintahSistem("netsh", "winsock reset", true);
                CetakSukses("DNS di-flush dan Network di-reset!");
            }
            if (pilihan == "8")
            {
                Console.ForegroundColor = ConsoleColor.Red; Console.WriteLine("[!] PERINGATAN: Proses ini memakan waktu 5 - 20 menit."); Console.ResetColor();
                Console.WriteLine("\nMenjalankan DISM RestoreHealth..."); JalankanPerintahSistem("DISM", "/Online /Cleanup-Image /RestoreHealth", false);
                Console.WriteLine("\nMenjalankan SFC Scannow..."); JalankanPerintahSistem("sfc", "/scannow", false);
                CetakSukses("System Repair Selesai!");
            }
            if (pilihan == "9")
            {
                AnimasiProgressBar("[~] Mengumpulkan HWID...");
                Console.ForegroundColor = ConsoleColor.Magenta; Console.WriteLine("\n=================== INFO HWID LO ==================="); Console.ResetColor();
                JalankanPerintahSistem("wmic", "baseboard get product,Manufacturer,version,serialnumber", false);
                JalankanPerintahSistem("wmic", "diskdrive get model,serialnumber", false);
                JalankanPerintahSistem("getmac", "", false);
                JalankanPerintahSistem("wmic", "csproduct get uuid", false);
                Console.ForegroundColor = ConsoleColor.Magenta; Console.WriteLine("===================================================="); Console.ResetColor();
                TampilkanPopUpWindows("Data HWID berhasil dimuat!", "Sukses");
            }
            if (pilihan == "11")
            {
                AnimasiProgressBar("[~] Membebaskan Memory RAM...");
                int dibersihkan = 0;
                foreach (Process proc in Process.GetProcesses()) { try { EmptyWorkingSet(proc.Handle); dibersihkan++; } catch { } }
                CetakSukses($"RAM di-flush dari {dibersihkan} aplikasi!");
            }
            if (pilihan == "12")
            {
                AnimasiProgressBar("[~] Membaca Windows Registry...");
                Console.Clear(); Console.ForegroundColor = ConsoleColor.Magenta;
                Console.WriteLine("=========================================================================================");
                Console.WriteLine("                        STARTUP ANALYZER (Aplikasi Booting)                              ");
                Console.WriteLine("=========================================================================================");
                Console.ResetColor();
                Console.WriteLine(string.Format("{0,-12} | {1,-25} | {2,-45}", "Status", "Nama Aplikasi", "Lokasi/Target"));
                Console.WriteLine("-----------------------------------------------------------------------------------------");
                ...
                Console.WriteLine("=========================================================================================");
                Console.ResetColor();
                TampilkanPopUpWindows("Analisis Startup Registry selesai!", "Sukses");
            }
            if (pilihan == "13")
            {
                Console.Clear();
                ...
                string subPilihan = BacaInputMenuIdle(); 
                string urlDownload = ""; string namaFile = "";
                ...
                {
                    string pathSimpan = Path.Combine(Environment.CurrentDirectory, namaFile);
                    Console.WriteLine($"\n[~] Mengontak Server..."); Console.ForegroundColor = ConsoleColor.Yellow;
                    Console.WriteLine($"[~] Mulai mendownload {namaFile}..."); Console.WriteLine($"[~] Pantau speed dan progres internet lo di bawah ini:"); Console.ResetColor();
                    DownloadFileDenganProgress(urlDownload, pathSimpan);
                }
            }
        ...
        while (true)
        {
            if (Console.KeyAvailable)
            {
                ...
                if (key.Key == ConsoleKey.Enter)
                {
                    ...
                    Console.Write("          "); Console.WriteLine(); Console.CursorVisible = true; return input;
                }
                else if (key.Key == ConsoleKey.Backspace && input.Length > 0)
                {
                    input = input.Substring(0, input.Length - 1); Console.SetCursorPosition(cursorLeft, cursorTop); Console.Write(input + "       "); 
                }
    ...
    // FUNGSI POP-UP WINDOWS NATIVE & SUARA SINGKAT
    static void TampilkanPopUpWindows(string pesan, string judul)
    {
        // Flag 0x00000040 (MB_ICONINFORMATION) otomatis memutar suara "Ting/Ding" default Windows!
        MessageBox(IntPtr.Zero, pesan, $"DWT Utility - {judul}", 0x00000040);
    }

    static void CetakSukses(string pesan)
    {
        Console.ForegroundColor = ConsoleColor.Green; Console.WriteLine($"\n[V] {pesan}"); Console.ResetColor();
        TampilkanPopUpWindows(pesan, "Berhasil");
    }
        ...
        try
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                using (HttpResponseMessage response = client.GetAsync(url, HttpCompletionOption.ResponseHeadersRead).GetAwaiter().GetResult())
                {
                    ...
                    {
                        byte[] buffer = new byte[8192]; bool isMoreToRead = true; long totalRead = 0;
                        Stopwatch timer = new Stopwatch(); timer.Start();
                        do
                        {
                            int read = contentStream.Read(buffer, 0, buffer.Length);
                            if (read == 0) { isMoreToRead = false; Console.WriteLine(); continue; }
                            ...
                            if (totalBytes.HasValue)
                            {
                                ...
                                int barSize = 30; int filled = (int)((progress / 100) * barSize);
            ...
            CetakSukses($"Download Sempurna! File siap di-install.");
        }
        catch (Exception ex) { Console.ForegroundColor = ConsoleColor.Red; Console.WriteLine($"\n[X] Gagal mendownload: {ex.Message}"); Console.ResetColor(); }
    }
        ...
        using (RegistryKey runKey = baseKey.OpenSubKey(runPath))
        {
            if (runKey != null)
            {
                foreach (string appName in runKey.GetValueNames())
                {
                    ...
                    string formatNama = appName.Length > 23 ? appName.Substring(0, 20) + "..." : appName;
                    string formatData = data.Length > 43 ? data.Substring(0, 40) + "..." : data;
                    if (status == "Enabled") Console.ForegroundColor = ConsoleColor.Green; else Console.ForegroundColor = ConsoleColor.DarkGray;
                    Console.WriteLine(string.Format("[{0,-10}] | {1,-25} | {2,-45}", status, formatNama, formatData));
                    Console.ResetColor();
                }
            }
        }
    ...
    static string CekStatusStartupApproved(RegistryKey baseKey, string appName)
    {
        try
        {
            using (RegistryKey approvedKey = baseKey.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\Run"))
            {
                if (approvedKey != null)
                {
                    object val = approvedKey.GetValue(appName);
                    if (val != null && val is byte[] bytes && bytes.Length > 0) { if (bytes[0] == 0x03 || bytes[0] % 2 != 0) return "Disabled"; }
                }
            }
    ...
    static void AnimasiProgressBar(string pesan)
    {
        ...
        {
            int filled = (i * barSize) / 100; string bar = new string('█', filled) + new string('-', barSize - filled);
            Console.Write($"\r[{bar}] {i}% "); Thread.Sleep(20); 
        }
        Console.WriteLine();
    }
    ...
    static void HapusFolderPaksa(string folderPath)
    {
        if (Directory.Exists(folderPath)) { try { Process proc = new Process(); proc.StartInfo.FileName = "cmd.exe"; proc.StartInfo.Arguments = $"/c rmdir /s /q \"{folderPath}\""; proc.StartInfo.UseShellExecute = false; proc.StartInfo.CreateNoWindow = true; proc.Start(); proc.WaitForExit(); } catch { } }
    }

    static void BersihkanFolder(string folderPath)
    {
    ...
    static void EksekusiScanFolder(string path, string judul)
    {
        ...
        Console.WriteLine("-----------------------------------------------------------------------------------------");
        foreach (var item in hasilUrut)
        {
            double ukuranGB = (double)item.Value / (1024 * 1024 * 1024);
            Console.WriteLine(string.Format("{0,-45} | {1,-15}", item.Key, ukuranGB.ToString("0.00") + " GB"));
        }
        Console.ForegroundColor = ConsoleColor.Magenta; Console.WriteLine("========================================================================================="); Console.ResetColor();
        TampilkanPopUpWindows($"Pencarian {judul} Selesai!", "Sukses");
    }

    static long AmbilUkuranFolder(string folderPath)
    {
        long totalSize = 0;
        try { var files = Directory.EnumerateFiles(folderPath, "*.*", SearchOption.AllDirectories); foreach (var file in files) { try { totalSize += new FileInfo(file).Length; } catch { } } } catch { }
        return totalSize;
    }
}